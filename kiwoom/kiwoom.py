...
class Kiwoom(QAxWidget):
    def __init__(self):
        ...
        self.account_num = None #계좌를 담아줄 변수
        self.deposit = 0 #예수금
    ...
    def get_ocx_instance(self):
        self.setControl("KHOPENAPI.KHOpenAPICtrl.1") # 레지스트리에 젖아된 api 모듈 불러오기
    ...
    def detail_account_info(self, sPrevNext="0"):
        ...
        self.dynamicCall("SetInputValue(QString, QString)", "비밀번호", "3849")
    ...
    def not_concluded_account(self, sPrevNext):
    ...
    def trdata_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        if sRQName == "예수금상세현황요청":
            ...
            total_buy_money = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, 0, "총매입금액") #출력 00000000746100
            self.total_buy_money = int(total_buy_money)
            total_profit_loss_money = self.dynamicCall("GetCommData(QString, QString, int, QString)",sTrCode, sRQName, 0 ,"총평가손익금액") #0000009761
            self.total_rofit_loss_money = int(total_profit_loss_money)
            total_profit_loss_rate = self.dynamicCall("GetCommData(QString, QString, int, QString)", sTrCode, sRQName, 0, "총수익률(%)") # 출력 : 00000 1.31
            self.total_profit_loss_rate = float(total_profit_loss_rate)

            print("계좌평가잔고내역용청 싱글데이터 : %s - %s - %s" % (total_buy_money, total_profit_loss_money,total_profit_loss_rate))
            ...
            rows = self.dynamicCall("GetRepeat(QString, QString)",sTrCode,sRQName)
            ...
            for i in range(rows):
                ...
                self.not_account_stock_dict[code].update({"종목코드":code})
                self.not_account_stock_dict[code].update({"종목명":code_nm})
                self.not_account_stock_dict[code].update({"주문번호":order_no})
                self.not_account_stock_dict[code].update({"주문상태":order_status})
                ...
                self.not_account_stock_dict[code].update({"미체결수량":not_quantity})
    ...
    def stop_screen_cancel(self, sScrNo=None):
        self.dynamicCall("Disconnect(RealData(QString)", sScrNo)
